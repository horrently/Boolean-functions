...
public class task7 extends Application {
    ...
    public void start(Stage primaryStage) {
        primaryStage.setTitle("Игра. КНФ.");
        ...
        Label dnfLabel = new Label("Введите КНФ для этого вектора функции:");
        TextArea dnfTextArea = new TextArea();
        Button checkDNFButton = new Button("Проверить КНФ");
        Label resultLabel = new Label();

        // Обработчик нажатия на кнопку проверки ДНФ
        checkDNFButton.setOnAction(event -> {
            String userDNF = dnfTextArea.getText().trim();
            String correctDNF = buildCNF(booleanFunction);
    ...
    public static String buildCNF(int[] booleanFunction) {
...
private static String buildTerm(int index, int numVariables) {
    ...
    for (int i = 0; i < numVariables; i++) {
        ...
        term.append(bit == 0 ? variable : "!" + variable);
    }
